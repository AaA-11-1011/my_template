...
# CyTeX テンプレートに Gemini CLI と Python を追加する方法

このガイドでは、既存の CyTeX 環境に Gemini CLI と Python 開発環境を後付けする手順を説明します。README.md などにコピペしてご利用ください。

---

## 1. Gemini CLI のインストール

ターミナルで以下のコマンドを実行します。root ユーザーでログインしているため `sudo` は不要です。
...
高速なパッケージマネージャー `uv` を使用したセットアップ手順です。
...
# インストールを反映させるため、一度シェルを再起動するか以下を実行
export PATH="/root/.local/bin:$PATH"

# 必要なライブラリ（Google GenAI など）のインストール
uv pip install --system google-genai
```

---

## 3. (推奨) Dockerfile を直接書き換える場合

最初からこれらが入った状態でコンテナを起動したい場合は、`.devcontainer/Dockerfile` を以下のように編集してください。

```dockerfile
FROM ran350/cytex-texlive:0.1.0
